...
public:
  ...
  {
    elements[0][0] = m.getRight ().x;
    elements[0][1] = m.getRight ().y;
    elements[0][2] = m.getRight ().z;
    elements[0][3] = 0.0f;
    elements[1][0] = m.getUp ().x;
    elements[1][1] = m.getUp ().y;
    elements[1][2] = m.getUp ().z;
    elements[1][3] = 0.0f;
    elements[2][0] = m.getBack ().x;
    elements[2][1] = m.getBack ().y;
    elements[2][2] = m.getBack ().z;
    elements[2][3] = 0.0f;
    elements[3][0] = v.x;
    elements[3][1] = v.y;
    elements[3][2] = v.z;
...
TEST_CASE ("MyMatrix4 constructors", "[Transform]")
{
  ...
  CHECK (t2.elements[0][0] == Approx (ARBITRARY_MATRIX.getRight ().x));
  CHECK (t2.elements[0][1] == Approx (ARBITRARY_MATRIX.getRight ().y));
  CHECK (t2.elements[0][2] == Approx (ARBITRARY_MATRIX.getRight ().z));
  CHECK (t2.elements[0][3] == Approx (0.0f));
  CHECK (t2.elements[1][0] == Approx (ARBITRARY_MATRIX.getUp ().x));
  CHECK (t2.elements[1][1] == Approx (ARBITRARY_MATRIX.getUp ().y));
  CHECK (t2.elements[1][2] == Approx (ARBITRARY_MATRIX.getUp ().z));
  CHECK (t2.elements[1][3] == Approx (0.0f));
  CHECK (t2.elements[2][0] == Approx (ARBITRARY_MATRIX.getBack ().x));
  CHECK (t2.elements[2][1] == Approx (ARBITRARY_MATRIX.getBack ().y));
  CHECK (t2.elements[2][2] == Approx (ARBITRARY_MATRIX.getBack ().z));
  CHECK (t2.elements[2][3] == Approx (0.0f));
  CHECK (t2.elements[3][0] == Approx (ARBITRARY_VECTOR.x));
  CHECK (t2.elements[3][1] == Approx (ARBITRARY_VECTOR.y));
  CHECK (t2.elements[3][2] == Approx (ARBITRARY_VECTOR.z));
...
TEST_CASE ("Transform functions for getting and setting components work", "[Transform]") {
  ...
  t.setPosition (ANOTHER_VECTOR.x, ANOTHER_VECTOR.y, ANOTHER_VECTOR.z);
  CHECK (t.getOrientation () == ANOTHER_MATRIX);
  CHECK (t.getRight () == ANOTHER_MATRIX.getRight ());
  CHECK (t.getUp () == ANOTHER_MATRIX.getUp ());
  CHECK (t.getBack () == ANOTHER_MATRIX.getBack ());
...
TEST_CASE ("Transform::getTransform() and getTransform(float[16]) work", "[Transform]") {
  ...
  float expectedArray[16] =
    { ARBITRARY_MATRIX.getRight ().x, ARBITRARY_MATRIX.getRight ().y, ARBITRARY_MATRIX.getRight ().z, 0.0f,
      ARBITRARY_MATRIX.getUp ().x, ARBITRARY_MATRIX.getUp ().y, ARBITRARY_MATRIX.getUp ().z, 0.0f,
      ARBITRARY_MATRIX.getBack ().x, ARBITRARY_MATRIX.getBack ().y, ARBITRARY_MATRIX.getBack ().z, 0.0f,
