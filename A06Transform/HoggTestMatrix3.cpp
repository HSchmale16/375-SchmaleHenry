...
TEST_CASE ("Matrix3 default constructor creates identity matrix", "[Matrix3]") {
  Matrix3 m;
  CHECK (m.getRight ().x == Approx (1.0f));
  CHECK (m.getRight ().y == Approx (0.0f));
  CHECK (m.getRight ().z == Approx (0.0f));
  CHECK (m.getUp ().x == Approx (0.0f));
  CHECK (m.getUp ().y == Approx (1.0f));
  CHECK (m.getUp ().z == Approx (0.0f));
  CHECK (m.getBack ().x == Approx (0.0f));
  CHECK (m.getBack ().y == Approx (0.0f));
  CHECK (m.getBack ().z == Approx (1.0f));
}
...
TEST_CASE ("Matrix3 9-parameter construtor copies correctly", "[Matrix3]") {
  ...
  Matrix3 m2 (0.0f, 1.0f, -1.0f,
	      0.1f, 0.2f, 0.3f,
	      -1.0f, 0.5f, 0.0f);
  CHECK (m1.getRight ().x == Approx (1.0f));
  CHECK (m1.getRight ().y == Approx (2.0f));
  CHECK (m1.getRight ().z == Approx (3.0f));
  CHECK (m1.getUp ().x == Approx (4.0f));
  CHECK (m1.getUp ().y == Approx (5.0f));
  CHECK (m1.getUp ().z == Approx (6.0f));
  CHECK (m1.getBack ().x == Approx (7.0f));
  CHECK (m1.getBack ().y == Approx (8.0f));
  CHECK (m1.getBack ().z == Approx (9.0f));
  CHECK (m2.getRight ().x == Approx (0.0f));
  CHECK (m2.getRight ().y == Approx (1.0f));
  CHECK (m2.getRight ().z == Approx (-1.0f));
  CHECK (m2.getUp ().x == Approx (0.1f));
  CHECK (m2.getUp ().y == Approx (0.2f));
  CHECK (m2.getUp ().z == Approx (0.3f));
  CHECK (m2.getBack ().x == Approx (-1.0f));
  CHECK (m2.getBack ().y == Approx (0.5f));
  CHECK (m2.getBack ().z == Approx (0.0f));
}
...
TEST_CASE ("Matrix3 3-parameter construtor copies correctly", "[Matrix3]") {
  ...
  Matrix3 m2 (r2, u2, b2);
  CHECK (m1.getRight ().x == Approx (r1.x));
  CHECK (m1.getRight ().y == Approx (r1.y));
  CHECK (m1.getRight ().z == Approx (r1.z));
  CHECK (m1.getUp ().x == Approx (u1.x));
  CHECK (m1.getUp ().y == Approx (u1.y));
  CHECK (m1.getUp ().z == Approx (u1.z));
  CHECK (m1.getBack ().x == Approx (b1.x));
  CHECK (m1.getBack ().y == Approx (b1.y));
  CHECK (m1.getBack ().z == Approx (b1.z));
  CHECK (m2.getRight ().x == Approx (r2.x));
  CHECK (m2.getRight ().y == Approx (r2.y));
  CHECK (m2.getRight ().z == Approx (r2.z));
  CHECK (m2.getUp ().x == Approx (u2.x));
  CHECK (m2.getUp ().y == Approx (u2.y));
  CHECK (m2.getUp ().z == Approx (u2.z));
  CHECK (m2.getBack ().x == Approx (b2.x));
  CHECK (m2.getBack ().y == Approx (b2.y));
  CHECK (m2.getBack ().z == Approx (b2.z));
}
...
TEST_CASE ("Matrix3 2-parameter + bool construtor builds correct axis without normalization", "[Matrix3]") {
  ...
  CHECK (m2.getBack ().y == Approx (b2.y));
  CHECK (m2.getBack ().z == Approx (b2.z));
  CHECK (m3.getRight ().x == Approx (r3.x));
  CHECK (m3.getRight ().y == Approx (r3.y));
  CHECK (m3.getRight ().z == Approx (r3.z));
  CHECK (m3.getUp ().x == Approx (u3.x));
  CHECK (m3.getUp ().y == Approx (u3.y));
  CHECK (m3.getUp ().z == Approx (u3.z));
  CHECK (m3.getBack ().x == Approx (b3.x));
  CHECK (m3.getBack ().y == Approx (b3.y));
  CHECK (m3.getBack ().z == Approx (b3.z));
}
...
TEST_CASE ("Matrix3 data function (non-const) works", "[Matrix3]") {
  ...
  *d = 10.0f;
  CHECK (m.getRight ().x == Approx (10.0f));
}
...
TEST_CASE ("Matrix3 getUp function works", "[Matrix3]") {
  Matrix3 m (1.0f, 2.0f, 3.0f,
	     ...
	     7.0f, 8.0f, 9.0f);
  Vector3 v = m.getUp ();
  CHECK (v.x == Approx (4.0f));
  CHECK (v.y == Approx (5.0f));
  CHECK (v.z == Approx (6.0f));
}
...
TEST_CASE ("Matrix3 getBack function works", "[Matrix3]") {
  ...
  CHECK (v.y == Approx (8.0f));
  CHECK (v.z == Approx (9.0f));
}
...
TEST_CASE ("Matrix3 setFromAngleAxis function works", "[Matrix3]") {
  ...
  Vector3 recoveredAxis(m.getUp ().z - m.getBack ().y,
			m.getBack ().x - m.getRight ().z,
			m.getRight ().y - m.getUp ().x);
  axis.normalize ();
  recoveredAxis.normalize ();
  CHECK (recoveredAxis == axis);
  float trace = m.getRight ().x + m.getUp ().y + m.getBack ().z;
...
TEST_CASE ("Matrix3 transform function works", "[Matrix3]") {
  ...
  Vector3 firstRow = Vector3 (m.getRight ().x, m.getUp ().x, m.getBack ().x);
  Vector3 secondRow = Vector3 (m.getRight ().y, m.getUp ().y, m.getBack ().y);
  Vector3 thirdRow = Vector3 (m.getRight ().z, m.getUp ().z, m.getBack ().z);
  CHECK (r.x == Approx (firstRow.dot (v)));
  CHECK (r.y == Approx (secondRow.dot (v)));
  CHECK (r.z == Approx (thirdRow.dot (v)));
}
